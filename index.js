
// variableslet randomNumber = 0;
let randomNumber = 0;
let roundScore = 0;
let activePlayer = 0;
let scores = [0, 0];

// DOM elements

// Get the #dice element
const dice = document.querySelector(".dice");
// Get roll button
const roll = document.querySelector("#roll--icon");
// Get hold button
const hold = document.querySelector("#save--icon");
// Get new game button
const newGame = document.querySelector("#newgame--icon");
// Get the players
const player1 = document.querySelector(".player--0");
const player2 = document.querySelector(".player--1");
// Get the score
const score1 = document.querySelector("#score--0");
const score2 = document.querySelector("#score--1");
// Get the current round score
const roundScore1 = document.querySelector("#current--0");
const roundScore2 = document.querySelector("#current--1");
// Event listeners

// Roll the dice and display the round score
const rollDice = function () {
  // Create a random number
  randomNumber = Math.floor(Math.random() * 6) + 1;
  // Display dice
  dice.innerHTML = `<img src="assets/images/dice/dice-${randomNumber}.png" alt="Playing dice ${randomNumber}" class="dice ${randomNumber}">`;
  // Add round score
  if (randomNumber !== 1) {
    roundScore += randomNumber;
  // Display round score
    document.querySelector(`#current--${activePlayer}`).textContent = roundScore;
  } else {
    changePlayer();
  }
};

// Change player
const changePlayer = function () {
  roundScore = 0;
  document.querySelector(`#current--${activePlayer}`).textContent = 0;
  activePlayer = activePlayer === 0 ? 1 : 0;
  player1.classList.toggle("active-player");
  player2.classList.toggle("active-player");
};

// Hold the score
const holdScore = function () {
  //add score
  scores[activePlayer] += roundScore;
  //display score
  document.querySelector(`#score--${activePlayer}`).textContent = scores[activePlayer];

  //check if player won
  if (scores[activePlayer] >= 100) {
    document.querySelector(`.name--${activePlayer}`).classList.add("winner-player");
    document.querySelector(`.name--${activePlayer}`).innerHTML = `<p>winner !</p>`;
    
  } else {
  //change player
  changePlayer();
  }
};

// New game
const replay = function () {
  document.location.reload();
};

// Event listeners
// Roll dice
roll.addEventListener("click", rollDice, false);
hold.addEventListener("click", holdScore, false);
newGame.addEventListener("click", replay, false);